...
class ItemService {
  ...
  async getProducts() {
    ...
    const resp = await axios.get(this.serverURL + "/products"); // using axios get request //must use await so the code does not run prematurely
    return resp.data;
  }
  //cart = array of products
  async getCart(userName, cart) {
    let data = {
      user: userName,
      total: 0,
      products: cart,
